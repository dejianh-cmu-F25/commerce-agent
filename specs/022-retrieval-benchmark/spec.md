...
- **Input distribution**: 27 labeled queries (easy/medium/hard) over the policy
  corpus; adversarial queries are not covered (gap).
- **Data quality**: the corpus is versioned in the repo; metrics are deterministic.
- **Edge & failure modes**: a config with no hits scores 0; the gate fails below the threshold.
- **Scale envelope**: the benchmark is keyless and offline; a large corpus is out of scope.
- **Degradation**: n/a (offline).
- **Change evidence**: this feature is the retrieval evidence (EV-1).
